...
class World:
    ...
    def toCarpedm(self):
        ...
        state = np.zeros((self.rows+2, self.cols+2), dtype = np.int8)        
        zero_state = np.tile(
        ...
        x,y = self.heroCol, (self.rows + 1 - self.heroRow)
        ...
        for r in range(self.rows+2):
            for c in range(self.cols+2):
                if (r == 0 or r == self.rows+1 or c == 0 or c == self.cols+1) or self.blocked[r-1][c-1] == 1:
                    state[self.rows + 1 - r][c][4] = 1
                elif int(self.markers[r-1][c-1]) > 0:
                    if int(self.markers[r-1][c-1]) > 9:
                        raise Exception(f"Number of markers are over the limit: {int(self.markers[r-1][c-1])}")
                    ## I guess this would accept maximum of 9
                    state[self.rows + 1 - r][c][int(self.markers[r-1][c-1]) + 5] = 1                              
        return state
    ...
    def toPytorchTensor(self, padding):
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                ...
                if nb_markers > 0:
                    tensor[5+int(nb_markers)][r+1][c+1] = 1
...
#             markers = np.concatenate((markers, (nb_marker_m1+1) * marker_map))
