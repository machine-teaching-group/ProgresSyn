...
def create_smaller_dataset(path_to_dataset, path_to_vocab, k):
    '''
...
#     path_to_ds_cache = path_to_dataset.replace('.json', '.thdump')
    path_to_ds_cache = f"dataset_{k}.thdump"
    if os.path.exists(path_to_ds_cache):
        dataset = torch.load(path_to_ds_cache)
    else:
        idxs = random.sample(list(range(1000000)), k)
        with open(path_to_dataset, 'r') as dataset_file:
            srcs = []
            tgts = []
            current_ios = []
            for i, sample_str in enumerate(tqdm(dataset_file.readlines())):
                if i in idxs:
...
def get_minibatch(dataset, sp_idx, batch_size,
    ...
    inp_grids = Variable(torch.stack(inp_grids, 0), requires_grad=not volatile_vars)
    out_grids = Variable(torch.stack(out_grids, 0), requires_grad=not volatile_vars)
    ...
    if volatile_vars:
        in_tgt_seq = torch.LongTensor(input_lines)#, requires_grad=not volatile_vars)
        out_tgt_seq = torch.LongTensor(output_lines)#, requires_grad=not volatile_vars)
    else:
#         print(f"type(input_lines)={type(input_lines)}")
        in_tgt_seq = torch.tensor(input_lines)#not volatile_vars)
        out_tgt_seq = torch.tensor(output_lines)
