...
def do_beam_rl(model,
    ...
    use_cuda = inp_grids.is_cuda
    tt = torch.cuda if use_cuda else torch
    vol_inp_grids = Variable(inp_grids.data, requires_grad=False)
    vol_out_grids = Variable(out_grids.data, requires_grad=False)
    ...
    for start_pos in range(0, len(decoded), rl_inner_batch):
        ...
        batch_reward += inner_batch_reward.detach().item()
