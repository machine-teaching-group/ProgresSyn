    ...
    if os.path.isdir(data_in_path):
        for filename in os.listdir(data_in_path):
            if( "code" in filename ):
                ...
                ast_in_path = os.path.join(data_in_path, filename)
                ...
                code_out_path = os.path.join(data_out_path, filename_txt)
                with open(code_out_path, "w") as f:
                    f.write(code)
    else:
        ast_in_path = data_in_path
        with open(ast_in_path) as f:
            ast = json.load(f)
        ...
        code = " ".join(tokens)
        #print("tokens:\n", tokens)
        filename_txt = os.path.split(ast_in_path)[-1].split(".")[0] + ".txt"
        code_out_path = os.path.join(data_out_path, filename_txt)
        with open(code_out_path, "w") as f:
            f.write(code)
